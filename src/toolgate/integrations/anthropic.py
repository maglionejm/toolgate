"""Anthropic tools-format adapter. Dependency-free: emits plain dicts in the
Messages API `tools=[...]` schema, plus a dispatcher that routes the model's
tool calls through the gate."""
...
def anthropic_tools(
    ...
    `tools` plugs into `client.messages.create(tools=...)`; `dispatch(name,
    input)` executes the model's tool call through the gate and returns a
    ...
    tools = [
        {
            ...
            "input_schema": t["argsSchema"] or {"type": "object"},
        }
